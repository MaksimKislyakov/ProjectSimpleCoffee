...
class ScheduleService:
    ...
    def _validate_schedule_data_start_time_more_end_time(
        self, schedule_data: ScheduleCreate
    ):
    ...
    def _validate_schedule_no_time_conflicts(
        ...
        for existing_schedule in all_schedules:
            ...
            if (
                ...
                and existing_schedule.coffee_shop_id == schedule_data.coffee_shop_id
            ):
                # Нормализуем даты существующей смены
                existing_start = self._normalize_datetime(
                    existing_schedule.schedule_start_time
                )
                existing_end = self._normalize_datetime(
                    existing_schedule.schedule_end_time
                )
    ...
    async def get_all_schedules_is_confirmed_false(self, current_user: User):
        ...
        all_schedules_is_confirmed_false = (
            await self.schedule_repo.get_all_is_confirmed_false()
        )
    ...
    async def update_schedule_actual_time(
        ...
        if actual_start_time is None and actual_end_time is None:
            raise HTTPException(
                status_code=400, detail="Укажите время начала или окончания"
            )
