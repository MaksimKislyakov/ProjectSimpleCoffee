...
from schemas.report_schemas import CreateReport
...
class ReportService:
    ...
    async def get_my_report_of_all_time(
        ...
        total_salary = CalculateServices._count_total_salary(
            current_user.hourly_rate, total_hours
        )

        # Получаем премии и штрафы за период
        total_award_and_fine = await self.reports_repo.get_total_adwards_and_fine(
            current_user.id, start_date, end_date
        )

        data_total_award_and_fine = self._filter_awards_and_fines_for_period(
            start_date, end_date, total_award_and_fine
        )

        total_award, total_fine = CalculateServices._count_total_award_and_fine(
    ...
    async def create_total_award_or_fine(self, data: CreateReport, current_user):
        ...
        new_report_award_or_fine = await self.reports_repo.create_total_adward_or_fine(
            report_award_and_fine
        )

        return new_report_award_or_fine

    def _filter_awards_and_fines_for_period(
        self, start_date, end_date, all_awards_and_fines
    ):
